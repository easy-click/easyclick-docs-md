...
- v3.0.0 新版本推荐使用HID小主机 [HID小主机](/docs/centerscreen/hidlinux) 免驱动 成本低 更方便
- v3.5.0+使用长链接组网，可以主控在云服务器，从控在本地局域网，可以互通, 也可以主控从控都在一个网络内组网
:::

:::tip
- HID主控 4.0.0+支持windows上的winusb驱动，可以不安装libusb驱动，但是如果使用zadig查看设备驱动不是winusb的，一定要替换为winusb才能运行
- 3.6.0,4.2.0版本分成了含键盘的和不含键盘的两种文件，是为了避免某些APP检测键盘模拟，使用无键盘的home等输入键无法使用
:::
## 下载软件
...
## Linux 平台
- Linux是免驱动的，无需安装驱动
- 解压zip文件后，在终端运行 **sudo ./hid-linux-v1.5.0** 二进制程序
...
- `device_info claim interface Error { kind: Unsupported, code: None, message: \"incompatible driver is installed for this device\" }`
  - 这个错误是因为驱动不正确，在windows上请使用winusb驱动，其他驱动可以会导致这个问题

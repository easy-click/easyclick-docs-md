...
keywords:
  - EasyClick 自动化脚本 android免root 代理事件
  - OCR
  - ocr
  - '9.17'
  - Tesseract
...
- 目前的OCR包含了mlkit,ocrLite,百度AI的easyedge,paddlOcrNcnnV5,paddleOcrOnnxV4，paddleOcrOnnxV5,paddleOcrOnnxV6,Tesseract,paddleOcrOnline和百度在线识别
- 支持PPOCR-V4、PPOCR-V5和PPOCR-V6模型（V6 需 Android 8.0/API26+，默认 modelTier=small）
...
* @param map map参数表
* key分别为：<br/>
* type : OCR类型，值分别为 OCR , paddleOcrNcnnV5 = ncnn版本的PPOCR-V5模型, paddleLiteOcr=paddleLite，  paddleOcrOnnxV4=onnx实现的PPOCR-V4模型，paddleOcrOnnxV5=onnx实现的PPOCR-V5模型， paddleOcrOnnxV6=官方ppocr-sdk+ORT实现的PPOCR-V6模型（API≥26，默认small）， tess = Tesseract模块，baiduOnline=百度在在线识别模块，paddleocr=百度离线的paddleocr，easyedge=百度AI OCR<br/>
...
* 如果参数是`paddleOcrOnnxV4`或者`paddleOcrOnnxV4`
  * 参数设置为 `{"type":"paddleOcrOnnxV4","numThread":2,"padding":50,"maxSideLen":960}`
  - 参数解释：
  - numThread 使用的CPU线程数，
  - modelsDir： 模型路径，如果是外部路径例如 /sdcard/models/代表是sdcard下面的，默认自带有模型 不写这一项即可
...
* 如果参数是`paddleOcrOnnxV6`
  * 参数设置为 `{"type":"paddleOcrOnnxV6","modelTier":"small","numThread":2,"padding":32,"maxSideLen":640}`
  - 需 Android 8.0（API ≥ 26）；低版本请用 paddleOcrNcnnV5 / paddleOcrOnnxV5
  - modelTier：模型档位，默认 `small`（`PP-OCRv6_small`）
  - numThread：CPU 线程数
  - padding：图像外接白框，默认 32
  - maxSideLen：按图像长边缩放，默认 640
...
### 12.4.0+版本 paddleOcrOnnxV6 例子

* 需 Android 8.0（API ≥ 26），默认 `modelTier=small`

```javascript showLineNumbers
let paddleOcrOnnxV6 = null
setStopCallback(function () {
  logi("释放 paddleOcrOnnxV6 对象")
...
function initOcrpaddle() {
  ...
  if (!paddleOcrOnnxV6.initOcr(paddleOnnxMap)) {
    loge("OCR初始化失败 : " + paddleOcrOnnxV6.getErrorMsg())
    exit()
  }
}
...
function ocrFunc() {
  ...
  if (result) {
    ...
    for (let i = 0; i < result.length; i++) {
      let value = result[i]
      logd("文字 : " + value.label + " confidence:" + value.confidence
        + " 范围: " + value.x + "," + value.y + ","

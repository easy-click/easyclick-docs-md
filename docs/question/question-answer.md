...
# 常见问题
## 12.0.0+以上版本问题
### 打开UI报错：执行UI错误:java.lang.RuntimeException: 【40009】无法匹配APK和脚本文件的发布选项属性
- ec的默认apk和打包的调试版本只能使用idea链接运行iec，不能使用idea 编译出来的release的iec脚本
- 打包的普通版本和企业版本，可以运行idea 编译的release版本iec
-
### 点击执行报错：执行错误:打开脚本文件失败: 【40009】 IEC debug=2 requires ProductInfo apk type 2 or 3, got 4
- 如果打包的是普通版本的apk或者企业版本的apk，并且链接了idea，这个时候apk变成了调试版本，只能链接使用idea发送过来的调试版本iec，不能使用idea 编译的iec
- 如果想变回普通版本或者企业版本，直接重启手机或者划掉app进程，重新启动，不要链接idea
...
- 3.X版本自动化服务是脚本运行的必要条件，4.x 将自动化服务作为非必要条件，在4.x中使用自动化服务请自行在脚本中判断或者启动服务环境，
  [启动环境](/docs/funcs/global/global.md#startenv)和[判断环境](/docs/funcs/global/global.md#isServiceOk)
...
- 请参考 [远程调试](/docs/funcs/devtools/dev-tools-remote.md)

...
## 下载安装
- 到教程和资源区下载 [教程和资源区](/docs/tools/download_resources)，可以使用一键安装包下载，或者手动下载
...
- 设置EC为默认输入法,查看 本章节的[打字输入](/docs/centerscreen/openscreen#打字输入) ，该步骤不是必须做的，是防止部分手机弹窗问题 
- 然后点击投屏操作即可,如果手机弹不出录屏授权，请将EC的app设置为默认输入法、打开悬浮窗，或者打开EC的app
- **在使用WIFI投屏前，请先先开启无障碍自动化，点击工具栏-开启自动化按钮**，你可以查看 [授权无障碍](/docs/centerscreen/openscreen#授权无障碍) 章节，给予自启无障碍权限，无需手动点击启动无障碍
- **在使用WIFI投屏前，可以先开启授权截图自允许**，你可以查看 [授权截图自允许](/docs/centerscreen/openscreen#授权截图自允许) 章节，给予自启`授权截图自允许`权限，投屏时不会弹出授权窗，系统自动允许
...
## HID投屏(USB-HID,蓝牙HID,OTG-HID)

- HID投屏这里特殊说明，这个HID指的是执行点击等替换为HID模式，这个时候不用`开启无障碍服务和adb调试模式`
- HID的投屏可以是`局域网WIFI投屏`和`广域网投屏`传递的投屏数据
- 这里支持三种HID,分别是**USB-HID,蓝牙HID,OTG-HID**
- USB-HID的配置推荐使用Linux主机带机[USB-HID主机配置](http://localhost:3000/docs/centerscreen/hidlinux)
- 蓝牙HID请参考[蓝牙HID硬件教程](/docs/advance/blehid)，配置好了之后再进行操作
- OTG-HID请参考[OTG-HID硬件教程](/docs/advance/otghid)
- 上面的HID，推荐使用OTG方式，配置简单，响应速度快

### OTG HID 点击
- OTG-HID请参考[OTG-HID硬件教程](/docs/advance/otghid)
- 按照上面的教程，将OTG配置好
- 到**投屏界面-左侧工具栏-系统设置-投屏配置**中，更改投屏方式为**OTG-HID**
- 下一步即可操作屏幕了
- - 如果不成功，就去**APP设置-OTG-HID 设置**测试一下功能是否正常

### 蓝牙 HID 点击
- 蓝牙HID请参考[蓝牙HID硬件教程](/docs/advance/blehid)，配置好了之后再进行操作
- 到**投屏界面-左侧工具栏-系统设置-投屏配置**中，更改投屏方式为**蓝牙-HID**
- 下一步即可操作屏幕了
- 如果不成功，就去**APP设置-蓝牙BLE设置**测试一下功能是否正常

### USB HID模式点击
- USB HID不用开启无障碍、USB调试模式，这里推荐使用 `Linux系统 进行安装 HID主控程序`，具体请看 [HID小主机](/docs/centerscreen/hidlinux)
- USB HID主控在Linux安装完成后，点击**投屏界面-左侧工具栏-USB HID主控**中进行**高级联网**，扫描到HID主机后再进行下一步
- 1、投屏页面-系统设置-投屏配置，更改点击方式为**USB-HID**
- <img src='/andqk/s2/1.png' style={{zoom:'30%'}}/>
- 2、点击工具栏激活USB HID，若投屏页面上提示 **激活HID成功** 代表可以使用HID模式点击了
- <img src='/andqk/s2/2.png' style={{zoom:'30%'}}/>
  <img src='/andqk/hid-3.png' style={{zoom:'30%'}}/>
- 3、如果不成功,请点击工具栏**HID主控** 查看设备列表是否已经激活HID、驱动是否安装完毕等
- **HID主控的安装、升级、以及组网等请查看**[HID小主机](/docs/centerscreen/hidlinux)章节

## 打字输入
:::tip
- 建议先设置好输入法，然后在插入USB，避免获取输入法包名不正确
- 如果设置好了输入法也不能输入，建议重启一次手机
:::

...
### getCliArgs 获取CLI命令行提交的参数

* 获取CLI命令行提交的参数
* 如果配合AI使用，可以优先从cli获取参数，如果获取不到在从其他地方获取
* 这样就能做到AI 自动传参，打包后从其他地方获取参数的AI自动测试
* 适合版本 EC 脱机版本 6.9.0+
* @return `{null|JSON}` JSON对象，如果是null说明不是CLI运行过来的
...
* 获取自动化引擎模式
* @return `{string}` single 单App模式 | dual 双App模式
...
* 设置自动化引擎模式
* @param mode `{string}` single 单App模式 | dual 双App模式
* @return `{boolean}` true
...
### setLogWindowForcePlaybackPaused 设置日志浮窗暂停状态

* 设置日志浮窗 PiP 是否强制为「暂停态」（低 CPU 模式）。
* 适配脱机版本7.3.0+
* 系统默认值是true
* @param forcePaused `{boolean}`
* true：始终向系统报告暂停，仅按刷新间隔刷帧，脚本运行时 CPU 占用低（推荐，默认）。
* false：随脚本运行同步为播放态（旧行为，脚本运行时 CPU 占用较高）。
...
### isLogWindowForcePlaybackPaused 查询日志浮窗暂停状态

* 查询日志浮窗是否处于强制暂停态（低 CPU 模式）。
* 适配脱机版本7.3.0+
* @return `{boolean}` true 暂停态
...
### setLogWindowInfoVisible 显示或隐藏日志浮窗自定义信息栏
...
function main() {
    ...
    // 再次设置需要展示的信息
    setLogWindowInfoText("账号: test001-test001-test001-test001fdasfdsfdsafdsafdsaf-fdsafdsafds 很好 \n金币: 999\n状态: 运行中\n1\n2", "", 12)
}

main()
```

### setLogWindowInfoText 设置日志浮窗自定义信息

* 设置日志浮窗自定义信息（显示在顶部状态栏下方，不受 log 输出影响）。
* 适配脱机版本7.3.0+
* @param text `{string}` 要展示的重要数据信息，支持 \\n 换行（最多 5 行）
* @param color `{string}` 可选，16 进制文字颜色，如 #FF6600；不传或传空则使用日志浮窗默认文字颜色
* @param fontSize `{number}` 可选，字体大小；传 0 或不传则使用默认字体大小（比日志文字小 1 号）
* @return `{boolean}` true 代表成功
...
### setLogRefreshInterval 设置日志浮窗刷新间隔

* 设置日志浮窗刷新间隔（秒）。日志写入缓存后，按该间隔同步到 PiP 小窗。
* 适配脱机版本7.3.0+
* @param intervalSec 刷新间隔，单位秒，有效范围 0.2~60，默认 1
* @return `{boolean}` true 代表成功，false 代表参数无效
...
* direction: 文字展示方向，1 代表竖屏展示 0 代表横屏
* showTag： 显示调试标志，1 是 2否
* fitWidth：文字宽度适配悬浮窗的宽度，自动缩放，1 是，0否
...
### startActiveMySelf 启动自激活

* 开始启动自激活环境，实现自激活后，可以点击代理IP图标进行启动
* 这个函数执行时间最大是30秒
* 具体请看文档：https://ieasyclick.com/iostjdocs/zh-cn/advance/activemyself
* 适配 EC 脱机版 6.0.0+
* 如果使用外部的VPN，需要将主程序放在前台运行，防止无法跳转到LocalDevVpn程序
* @param openExtVpn 字符串，1 代表 使用外部的LocalDevVpn程序，0 代表使用内部的自带VPN
* @return `{string}` 返回ok代表成功，其他都是错误信息
...
### startDebugServer 开启调试服务

* 已经打包的也可以开启调试服务，给idea链接
...
### isReleaseIec 是否是发布版本

* 是否是发布版本的iec脚本
* 适配EC 脱机版本 5.9.0+
* @return boolean true是release，其他都是debug版本

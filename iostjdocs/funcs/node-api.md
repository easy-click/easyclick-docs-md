...
- 也可以使用设置节点获取参数的方式进行
- **不适合的页面：比如适配播放页面等，7.1.0以后支持了**

:::tip 特别说明
- 如果使用7.1.0以上的版本，并且使用单App模式的自动化引擎，可以支持所有app的页面节点
- 如果想加快速度，就使用setFetchNodeParam设置参数节点，需要设置过滤部分参数
- 例如：`setFetchNodeParam({"labelFilter":"1","maxChildCount":"200","maxDepth":"50","visibleFilter":"1","boundsFilter":"1","excludedAttributes":"visible,selected,enable,accessible"})`
- 这样设置后抓取视频页面速度在毫秒级别，测试系统为iOS18，iOS15系统可能不行
:::
...
## createSession 创建自动化session

* 创建自动化 session（抓节点前调用，理论上可以增加速度抓节点速度）
* @param bundleId 目标 App 的 bundleId；传 "" 则依次用上次 appLaunch、当前前台 App

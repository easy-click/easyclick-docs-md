...
- `ignoreSSLErrors` 为可选参数，不传或传 `false` 时行为与旧版一致；传 `true` 时可访问 HTTPS 证书过期、自签名等站点
- `http.request` / `http.requestEx` 支持 `method` 为 `GET`、`POST`、`PUT`、`DELETE` 等；`DELETE` 可将 `data` 拼到 URL 查询参数，或通过 `requestBody` 发送请求体
...
* @param param map参数，包含的参数有<br/>
    ...
    * method: GET、POST、PUT、DELETE 等 HTTP 方法字符串<br/>
    * proxy: 代理地址，map参数 包含主机和端口 例如 ```{"host":"11","port":111}```<br/>
    * followRedirects:是否自动跳转 true 或者 false<br/>
    * ignoreSSLErrors:是否忽略 HTTPS 证书错误（过期、自签名等）true 或者 false，默认 false<br/>
    ...
    * header:  HTTP 请求头，map参数,例如 ```{"UA":"test"}```
      * 如果使用application/x-www-form-urlencoded的content-type,参数是使用&分割的 可以将requestBody设置为请求参数例如 a=1&b=2,也可以升级最新版本，使用data参数传输map参数
      * 如果不设置content-type，method=POST或者PUT，content-type是multipart/form-data
...
* @param ignoreSSLErrors 是否忽略 HTTPS 证书错误（过期、自签名等），可选，默认 false
...
```
:::tip
- websocket链接最好用子线程，接收的数据放到共享存储区，需要5.20.0+版本
- 通过共享存储读写，做到JSVM互不干扰
- 下面是例子
:::
```javascript showLineNumbers
// js/websocket.js 文件内容
function testwebsocket() {
  let result = [];
  //新建一个ws连接,不要填127.0.0.1或localhost,脚本在手机上,127是手机ip,不是电脑
...
function threadTest() {
  ...
  th.execAsync(function () {
    ...
    // testwebsocket 是 websocket.js 文件中定义的 ，加上执行函数
  ...
  while (!isScriptExit()) {
    ...
    if (kvValue != "") {
      logd("获取当前时间: currentTime " + kvValue)
    }
    // 从数组共享区取值
...
* 开始异步连接
* 详细代码看[例子](/iostjdocs/funcs/http-api.md#httpnewwebsocket-websocket通信)
...
* 设置自动重连
* 详细代码看[例子](/iostjdocs/funcs/http-api.md#httpnewwebsocket-websocket通信)http-api

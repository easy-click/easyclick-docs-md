...
- 形态与 [iOS USB 蓝牙 BLE](/iosdocs/advance/ios-usb-ble) 一致：手机只当 **HID Host**；电脑/中控经串口或 WiFi 连接 ESP32 开发板，板再对手机发送蓝牙 HID
...
- 脚本 API 见 [蓝牙 BLE 事件](/hmdocs/funcs/ble-event-api)
...
- 刷入步骤与安卓一致，参考 [安卓蓝牙刷入固件](/docs/advance/blehid#刷入固件)
...
- 系统键也可在测试面板或脚本中调用：`home` / `back` / `recents`（最近任务）等，详见 [蓝牙 BLE 事件](/hmdocs/funcs/ble-event-api)
...
- 完整函数说明见 [蓝牙 BLE 事件](/hmdocs/funcs/ble-event-api)
- 对比 USB 有线 HID 方案见 [USB HID 事件](/hmdocs/funcs/hid-event-api)

...
keywords:
  - EasyClick 自动化脚本 鸿蒙Next自动化 全局快捷事件 资源下载
  - UI
  - boolean
  - 'null'
  - readAllUIConfig2
...
* 输入文字
* 适配 EC 鸿蒙Next 1.0.0+
* **输入法说明**：英数可走按键注入；中文等非 ASCII 内容由框架经**剪贴板 + 粘贴**写入当前焦点输入框。请将手机当前输入法设为**系统默认输入法**。若使用讯飞等第三方输入法，软键盘弹出时常见「内容只进剪切板、进不了输入框」；切回系统输入法后一般可正常进框。已连接蓝牙 HID 键盘时系统常不弹软键盘，代理输入相对更稳；需要稳定中文且保留第三方输入法时，可改用蓝牙 HID 输入（如 `bleEvent`）。
* @param content 内容
* @return `{bool}` true 成功，false 失败
...
* 适配 EC 鸿蒙Next 1.0.0+
* 具体码值请看 [按键值](/hmdocs/advance/keycode)
* @param key1 键1

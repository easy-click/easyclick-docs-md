...
keywords:
  - EasyClick 自动化脚本 鸿蒙Next自动化 OCR识别 资源下载
  - ocr
  - OCR
  - ocrLite
  - initOcr
...
- 目前的OCR包含了 ocrLite,TesseractOCR,paddleocr,paddleOcrOnnxV4
...
* @param map map参数表
* key分别为:
    * type : OCR类型，值分别为  paddleOcrOnnxV4 = PPOCR-V4模型， paddleOcrOnnxV5 = PPOCR-V5模型， paddleOcrOnnxV6 = PPOCR-V6_small 模型， ocrLite = ocrLite 模块， tess = tesseractOcr，paddleOcr=百度的飞桨OCR
* 如果类型是 `ocrLite`
...
* 如果类型是 `paddleOcrOnnxV6`
  * 内置 **PP-OCRv6_small**（det/rec ONNX），与 `paddleOcrOnnxV5` 并存；cls 复用 v5
  * 参数字段与 `paddleOcrOnnxV5` 相同，type 改为 `paddleOcrOnnxV6` 即可，例如：`{"type":"paddleOcrOnnxV6","numThread":2,"single":0,"matMode":1}`
  * 默认模型（可不写）：`PP-OCRv6_small_det.onnx` / `PP-OCRv6_small_rec.onnx` / `ppocrv6_small_labels.txt`
  * **偏快**：日常正立 UI 可用 `maxSideLen:640`、`doAngleFlag:0`、`mostAngleFlag:0`
* 如果类型是:`tess`
...
* 参数字段与 `paddleOcrOnnxV5` 相同；不写模型路径时使用中控自带模型
* **偏快**：日常正立 UI 可用 `maxSideLen:640`、`doAngleFlag:0`、`mostAngleFlag:0`
...
### PaddleOcrNcnnV5 OCR例子[2.12.0之后]

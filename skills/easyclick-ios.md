---
name: easyclick-ios
description: EasyClick iOS自动化开发Skill，包含USB版和脱机版两种版本、多种运行模式（代理/蓝牙HID/OTG HID/自主激活/激活器）、H5 UI规范、iOS专用选择器等iOS特有内容。当用户进行iOS平台EC开发时使用。
---
...
**开发步骤**：
1. **环境准备**
   - 安装IntelliJ IDEA（推荐 **2026.2 及以上**：无需激活 IDEA，直接安装开发插件即可使用）+ EasyClick iOS插件
   - iOS设备安装（根据模式选择）：

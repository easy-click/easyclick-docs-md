...
- 目前的OCR包含了 ocrLite、TesseractOCR、paddleOcr、paddleNcnnOcrV5、paddleOcrOnnxV4、paddleOcrOnnxV5、**paddleOcrOnnxV6**（PP-OCRv6_small）
...
* @param map map参数表
* key分别为:
    * type : OCR类型，值分别为 paddleNcnnOcrV5=NCNN版本的PPOCR-V5  paddleOcrOnnxV4 = PPOCR-V4模型， paddleOcrOnnxV5 = PPOCR-V5模型， paddleOcrOnnxV6 = PPOCR-V6_small 模型， ocrLite = ocrLite 模块， tess = tesseractOcr，paddleOcr=百度的飞桨OCR
* 如果类型是 `ocrLite`, 
...
* 如果类型是 `paddleOcrOnnxV4` 或者 `paddleOcrOnnxV5`, 
  ...
  * baseDir: 类库存放路径，一般会和ec自带在同个目录下,文件夹名称是: PaddleOcrOnnx，可以不写
...
* 如果类型是 `paddleOcrOnnxV6`
  * 内置 **PP-OCRv6_small**（det/rec ONNX），与 `paddleOcrOnnxV5` 并存，互不影响；角度分类 cls 复用 v5 模型
  * 参数字段与 `paddleOcrOnnxV5` 相同，type 改为 `paddleOcrOnnxV6` 即可，例如：`{"type":"paddleOcrOnnxV6","numThread":2,"single":0,"matMode":1}`
  * 默认模型文件名（可不写）：`PP-OCRv6_small_det.onnx` / `PP-OCRv6_small_rec.onnx` / `ppocrv6_small_labels.txt`；cls 仍为 `ch_ppocr_mobile_v2.0_cls_infer.onnx`
  * **语种**：同一模型可识别中文简繁、英文、日文及多种拉丁语系文字；西里尔文（如乌克兰语）不在支持范围
  * **偏快**：日常正立 UI 截图可用较小 `maxSideLen`（如 640）并关闭方向检测 `doAngleFlag:0`、`mostAngleFlag:0`；画面可能倒置时再打开方向检测
* 如果类型是:`tess`
...
function main() {
    ...
    // 也可以改成 paddleOcrOnnxV5 使用 PPOCR-V5
    let paddleOcrOn = {
...
### paddleOcrOnnxV6 例子（PP-OCRv6_small）

* type 使用 `paddleOcrOnnxV6`，内置 PP-OCRv6_small；与 `paddleOcrOnnxV5` 并存
* 参数字段与 `paddleOcrOnnxV5` 相同；不写模型路径/文件名时使用中控自带模型
* **语种**：中文简繁、英文、日文及多种拉丁文字；西里尔文（如乌克兰语）不支持
* **偏快**：日常正立 UI 可用 `maxSideLen:640`、`doAngleFlag:0`、`mostAngleFlag:0`（见下方 `paddleOcrOnnxV6Fast`）
...
function main() {
    ...
    // 默认偏精度（不写 maxSideLen / 方向检测时走引擎默认）

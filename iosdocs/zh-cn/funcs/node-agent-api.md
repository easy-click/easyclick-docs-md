...
:::tip 与 lockNodeFromXml 的区别
- **nodeAgent**：`lockNode` / `getNodeInfo` 在手机端完成，适合纯 WiFi 远程选节点
- **auxEvent + lockNodeFromXml**：`agentDumpXml` 经 Aux 拉 XML，再由 PC 侧 `lockNodeFromXml` 注入缓存，适合想用 [节点函数](./node-api.md) 在电脑侧做选择器匹配的场景
:::

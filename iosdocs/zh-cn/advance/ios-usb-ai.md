...
本文介绍在 **Cursor / IDEA 里编写 EC 脚本**。若您 **不想写代码**，请使用新中控内置的 **[AI 智能体](./ai-agent)**（中文对话 + 可视化工作流）。

:::

:::tip

- 该教程基于 9.27.0+ 开发插件版本，中控任何版本都支持
- 编程工具使用的是Cursor和IDEA
- IDEA作为全功能版本，包含了新建工程等，Cursor作为辅助编程、自动纠错和编译运行工具
- 所有大模型都可以支持，不仅限于Cursor等，配置方式都是类似
- iOS USB版本和脱机版本 使用的方式是相同的，唯一区别是 iOS USB版本不能预览UI，脱机版本可以
  :::
...
- 如果没有出现，**ec_work_config/ios/bin**目录，可以关闭 **IDEA** 重新打开工程文件夹
- **ec_work_config/ios/bin**存放的是EC IOS 编译相关的**CLI**和**SKILL.md**
...
- 注意：iOS USB版本工程无法预览，只有iOS脱机版本才能预览

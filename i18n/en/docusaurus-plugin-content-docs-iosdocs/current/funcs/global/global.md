...
keywords:
 - EasyClick automation scripts iOS no jailbreak global module
 - jar
 - JAVA
 - return
 - JS
...
### ipaVersion Get IPA version {#ipaversion-获取ipa版本}

* Get IPA version
* Requires EC iOS USB 6.6.0+
* @return `{string}` e.g. 6.6.0
...
* On iOS 17+, can start but cannot auto-close previous tunnel.
* If automation drops, call startTunnel first, then this function
* @return true or false

```javascript showLineNumbers
function main() {
    // iOS 17+: use this to restart tunnel
    startTunnel();
  var result = startEnv();
}
...
main();
 ```
### startTunnel Start tunnel {#starttunnel-启动隧道}
* Start tunnel
* Requires EC iOS USB 9.22.0+
* Closes previous tunnel; effective on iOS 17+
* @returns `{string}` empty = success, else failure
```javascript showLineNumbers
function main() {
    var result = startTunnel();
    logd(result)
}
...
### closeTunnel Close tunnel {#closetunnel-关闭隧道}
...
* @return `{boolean}` true = not expired, false = expired
...
* @param type 1 = control center device auth, 2 = screen mirroring auth
* @return `{string}` JSON string; useEcid: 1 = ECID auth, else device ID auth; isExp: 1 = expired; expTime = expiry time

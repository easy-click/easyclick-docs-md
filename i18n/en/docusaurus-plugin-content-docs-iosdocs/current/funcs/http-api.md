 ...
 * proxy: proxy address as a map with host and port, e.g. ```{"host":"11","port":111}```<br/>
...
* HTTP GET request
* @param url request URL
* @param params parameter map, e.g. ```{"a":"11"}``` or a plain string
* @param timeout timeout in milliseconds
* @param headers header map, e.g.```{"a":"11"}```
* @return string response body
...
* HTTP POST request
* @param url request URL
* @param params parameters, e.g. ```{"a":"11"}``` or a plain string
* @param files files to upload, e.g. ```{"file1":"D:/1.png"}```
* @param timeout timeout in milliseconds
* @param headers header map, e.g.```{"a":"11"}```
* @return string response body
...
## http.agentHttpGetString Proxy IPA GET Request

* Send an HTTP GET request via proxy IPA; currently limited to localhost (127.0.0.1) — external network requests are not supported
* Suitable for device-modification software requests
* Supported EC USB 7.21.0+
* @param url request URL
* @param timeout timeout in milliseconds, e.g. 10000
...
## http.agentHttpPostJson Proxy IPA Send JSON

* Send JSON via proxy IPA; currently limited to localhost (127.0.0.1) — external network requests are not supported
* Suitable for device-modification software requests
* Supported EC USB 7.21.0+
* @param url request URL
* @param timeout timeout in milliseconds, e.g. 10000
* @param dataMap JavaScript map data, e.g. `{"a":1}`
* @return `{string}` response string
...
## http.agentRequestEx Proxy IPA HTTP Request

* Send an HTTP request via proxy IPA; currently limited to localhost (127.0.0.1) — external network requests are not supported
* Suitable for device-modification software requests
* Supported EC USB 8.16.0+
* @param dataMap JavaScript map of forwarded data; see the `http.request` param table — same as `http.request` except file upload is not supported
* @return `{null|Response1}` response result

```javascript showLineNumbers
function main() {
 let requestBody2 = JSON.stringify({"A": 111});
 let cookie2 = {
 "ck": "tst1",
 "cookie2": "tst2"
 };
 ...
 let da2 = http.agentRequestEx(data2)

 logd(JSON.stringify(da2))
 logd("cookie2 "+JSON.stringify(da2["cookie"]))
 logd("header2 "+JSON.stringify(da2["header"]))
 logd("contentType2 "+da2["contentType"])
 logd("statusCode2 "+da2["statusCode"])
 logd("body2 "+da2["body"])
...
## http.agentForwardRequest Proxy IPA Forward HTTP

* Forward HTTP requests via proxy IPA
* This method forwards from USB to proxy IPA, then to the main app, and finally to an external URL through multiple hops
* Use with File Transfer Assistant or the standalone main app, e.g. to get the device external IP
* The device must have File Transfer Assistant or the standalone main app installed
* Supported EC USB 8.16.0+
* @param url forwarding URL on the standalone main app: http://127.0.0.1:18924/devapi/requestEx
* @param timeout timeout in milliseconds, e.g. 10000
* @param dataMap JavaScript map of forwarded data; see the `http.request` param table — same as `http.request` except file upload is not supported
* @return `{null|Response1}` response result
...
```javascript showLineNumbers
function main() {
 let url = "http://127.0.0.1:18924/devapi/requestEx"
 let requestBody = JSON.stringify({"A": 111});
 let cookie = {
 "ck": "tst1",
 "cookie2": "tst2"
 ...
 let data = {
 "method":"POST",
 "url":"http://baidu3.com?"+new Date(),
 "cookie":cookie,
 "requestBody":requestBody
 }

 let da = http.agentForwardRequest(url,2000,data)

 logd(JSON.stringify(da))
 logd("cookie "+JSON.stringify(da["cookie"]))
 logd("header "+JSON.stringify(da["header"]))
 logd("contentType "+da["contentType"])
 logd("statusCode "+da["statusCode"])
 logd("body "+da["body"])
...
* See [example](/iosdocs/funcs/http-api#httpnewwebsocket-websocket-communication)
